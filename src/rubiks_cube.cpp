...
public:
    ...
    // constructor used by the algorithm that checks if there are other cubes like it on memory, only needs 'sides'
    Cube(uint8_t ext_sides[6][N][N])
    {
        int i, j, k;
...
// analyzes if the cubes are exactly equal to one another, not counting different orientations
bool cubeIsEqual(uint8_t c1[6][N][N], uint8_t c2[6][N][N])
{
    int i,j,k;
...
bool cubeExists(uint8_t sides[6][N][N], list<Cube>* pastStates)
{
    int i;
    Cube c(sides);

    for (list<Cube>::iterator it = (*pastStates).begin(); it != (*pastStates).end(); ++it) {
        if(cubeIsEqual(c.sides, (*it).sides))
            return true;
        ...
        //c.rot2();c.rot3i(); // RL' would return it to original position
    }
...
void BFS(list<Cube>* processing, list<Cube>* pastStates)
{
    ...
    for(i=0; i<12; i++)
    {
        if(i != prevRot) // avoids making a copy of the previous state
        {
            ...
            if(!cubeExists(newCube.sides, pastStates))  // this slows the loop a lot
...
void AI_loop(Cube initial) 
{
    ...
    while(!processing.empty())
    {
        ...
        // memory used by pastStates
        int mem = pastStates.size();
        if(mem%1000==0)
            cout << mem << endl;
        // REMOVE
        if(mem > 39089245) // 271453 for 5 rots, 3257437 for 6 and 39089245 for 7
        {
            cout << "some error" << endl;
            return; // this returns because I am testing a cube with 5 rotations done to it
                    // one way to solve a 5-times rotated cube is by doing the 5 reversed rotations
                    // so no more than 271453 cube states are needed for that
        }
    }
